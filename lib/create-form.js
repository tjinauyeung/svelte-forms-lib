...
const isInitilaValueChangedDef = (initial, current) => current !== initial;
...
export const createForm = (config) => {
  ...
  const isInitilaValueChanged = config.isInitilaValueChanged || isInitilaValueChangedDef;
  ...
  const modified = derived(form, ($form) => {
    ...
    for (let key in $form) {
      if( isInitilaValueChanged(initialValues[key], $form[key]) ) {
        object[key] = true;
      }
    }
